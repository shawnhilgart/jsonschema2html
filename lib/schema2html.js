...
Schema2Html.prototype.buildForm = function(callback) {
    ...
    this.resolveDependencies(this.schema, function(err) {
...
Schema2Html.prototype.renderLoop = function(schema, depth, scopeName) {
    schema = schema || {};
    var type = schema.type || null,
        id = schema.id || scopeName,
        execProcess;

    id = this.generateId(id);
    type = schema.$ref ? 'ref' : type;

    execProcess = typeof (this.lookupTable[type]) === 'function' ? this.lookupTable[type] : null;
...
/**
 * @memberOf Schema2Html
 * @description Loads template files based on path
 *
 * @param {string} key internal key to cache template at
 * @param {string} path path to load template from
 * @param {callback}
 */

Schema2Html.prototype.loadTemplate = function(key, path, callback) {
    var _that = this;
    ...
    fs.readFile(__dirname + '/' + path, function(err, data) {
        ...
        if (err !== null) {
            ...
            _that.templates[key] = html = handlebars.compile(data.toString());
        }
...
/*
    Dependecy resolution logic below
 */
...
Schema2Html.prototype.resolveSchemaDependency = function(ref, schema, callback) {
    var _that = this;
...
Schema2Html.prototype.resolveDataDependency = function(ref, callback) {
    var _that = this,
        handler,
        url,
        type,
    ...
    handler = ref.split('://');
    type = handler[0];
    url = allowed[type] === true ? ref : handler.slice(1, handler.length);
    ...
    if (typeof resolver === 'function') {
        ...
        callback(new Error('No way to resolve data source:' + ref));
    }
};
...
Schema2Html.prototype.resolveDependencies = function(schema, callback) {
    ...
    q = async.queue(function(payload, asyncCallback) {
        ...
        if (schema.type === 'object') {
            ...
            if (_.isArray(schema.anyOf) && schema.anyOf.length > 0) {
                ...
                schema.anyOf.forEach(function(dependency) {
                    ...
                    if (dependency.$ref) {
                        ...
                        _that.resolveSchemaDependency(dependency.$ref, root, function(err, cached, result) {
                            ...
                            if (total === count) {
                                asyncCallback(); // resolved all of our references, callback and exit
                            }
                        });
                    } else {
                        asyncCallback(); // no reference callback and exit
                    }
                });
                ...
                _.forOwn(schema.properties, function(property) {
                    ...
                    q.push({schema: property, root: root}, function(err) {
            ...
            if (opts.datasrc) {
                ...
                // single dependency
                _that.resolveSchemaDependency(schema.$ref, root, function(err, cached, result) {
                    if (err !== null) {
                        callback(err);
                    } else {
                        asyncCallback();
    ...
    q.drain = function() {
        callback(null); // empty dependency queue, callback
    };
