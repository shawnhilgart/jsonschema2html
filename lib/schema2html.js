...
Schema2Html.prototype.renderLoopArray = function(id, name, depth, schema) {
    ...
    for (i = 0; i < itemsToRender; i += 1) {
        if (schema.items.$ref) {
            ...
            schema.items.anyOf.forEach(function(val, index) {
                ref = val.$ref;
                matchVal = _that.retrieveValue(_that.data, (name + '[' + i + ']' + '[' + match + ']'));

                if (ref === matchVal) {
                    _that.renderLoop(_that.dependencyCache[ref], (depth + 1), (name + '[' + i + ']'));
                }
